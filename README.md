...
- OIDC back-channel logout delivery using logout tokens
...
### Logout Token (JWT, Back-Channel Logout)

Used for server-to-server back-channel logout notifications sent to a client's registered `backchannel_logout_uri`.
...
- `GET /profile.html` - Static dashboard/profile page served after `/dashboard` validates the session.
...
- `POST /logout` - Ends web session and attempts back-channel logout delivery when configured.

## Educational Limits Of This Project

This project is intentionally scoped for learning, not production. I have made an effort to read the documentation and implement this, the tough part was to keep tokens on the backend while the frontend only holds an HttpOnly session cookie. This project was a way to gain understaning of the provider side of IdP.
